...
@C    scale_id106 => Material.TextureScale.i1
@C    offset_id107 => Material.TextureOffset.i1
@C    constantFloat_id108 => Material.GlossinessValue
@C    scale_id115 => Material.TextureScale.i2
@C    offset_id116 => Material.TextureOffset.i2
...
@R    Sampler_id99 => Material.Sampler.i0 [Stage: None, Slot: (-1--1)]
@R    Texture_id104 => Material.GlossinessMap [Stage: None, Slot: (-1--1)]
@R    Texture_id104 => Material.GlossinessMap [Stage: None, Slot: (-1--1)]
@R    Texture_id113 => Material.MetalnessMap [Stage: None, Slot: (-1--1)]
@R    Texture_id113 => Material.MetalnessMap [Stage: None, Slot: (-1--1)]
...
@R    Texture_id104 => Material.GlossinessMap [Stage: Pixel, Slot: (1-1)]
@R    Texture_id113 => Material.MetalnessMap [Stage: Pixel, Slot: (2-2)]
...
//
#line 664 "C:\_Rapporter-Xenko3D\Rapporter\Rapporter\Rapporter.Windows\log\shader_XenkoEditorForwardShadingEffect_91c150c04660bf3ba41517241f4a5eb3.hlsl"
...
//   float2 scale_id106;                // Offset:   16 Size:     8
//   float2 offset_id107;               // Offset:   24 Size:     8
//   float constantFloat_id108;         // Offset:   32 Size:     4
//   float2 scale_id115;                // Offset:   36 Size:     8
//   float2 offset_id116;               // Offset:   48 Size:     8
//
// }
//
// cbuffer PerView
...
// Texture_id104                     texture  float4          2d    1        1
// Texture_id113                     texture  float4          2d    2        1
...
//
#line 693 "C:\_Rapporter-Xenko3D\Rapporter\Rapporter\Rapporter.Windows\log\shader_XenkoEditorForwardShadingEffect_91c150c04660bf3ba41517241f4a5eb3.hlsl"
...
sample_indexable(texture2d)(float,float,float,float) r0.w, r3.xyxx, t1.yzwx, s0  // r0.w <- <Compute_id220 return value>.x
...
mul r0.w, r0.w, r0.w
mad r0.w, r0.w, r3.x, r1.w  // r0.w <- <BasicBlend_id400 return value>.x

#line 469
mad r3.xy, v3.xyxx, cb0[2].yzyy, cb0[3].xyxx
sample_indexable(texture2d)(float,float,float,float) r1.w, r3.xyxx, t2.yzwx, s0  // r1.w <- <Compute_id226 return value>.x
...
mov r5.w, l(0)
ld_indexable(texture3d)(uint,uint,uint,uint) r4.xy, r5.xyzw, t3.xyzw  // r4.x <- streams.lightData_id83.x; r4.y <- streams.lightData_id83.y
...
const static bool TInvert_id111 = false;
const static bool TIsEnergyConservative_id120 = false;
static const float PI_id122 = 3.14159265358979323846;
Texture2D Texture_id98;
SamplerState Sampler_id99;
Texture2D Texture_id104;
Texture2D Texture_id113;
...
float NormalDistributionGGX_id175(float alphaR, float nDotH)
{
    ...
    return alphaR2 / (PI_id122 * pow(max(nDotH * nDotH * (alphaR2 - 1) + 1, 0.0001), 2.0f));
}
...
float4 BasicBlend_id400(float4 backColor, float4 frontColor, float3 interColor)
{
...
{
    return float4(constantFloat_id108, constantFloat_id108, constantFloat_id108, constantFloat_id108);
}
float4 Compute_id220(inout PS_STREAMS streams)
{
    return Texture_id104.Sample(Sampler_id99, streams.TexCoord_id100 * scale_id106 + offset_id107).r;
}
...
float3 ComputeDirectLightContribution_id364(inout PS_STREAMS streams)
{
    ...
    float normalDistribution = Compute_id286(streams);
...
{
    float3 diffuseColor = streams.matDiffuseVisible_id62;
    return diffuseColor / PI_id122 * streams.lightColorNdotL_id40 * streams.matDiffuseSpecularAlphaBlend_id58.x;
}
...
{
    return Texture_id113.Sample(Sampler_id99, streams.TexCoord_id100 * scale_id115 + offset_id116).r;
}
float4 Compute_id225(inout PS_STREAMS streams)
{
    float4 backColor = Compute_id220(streams);
    float4 frontColor = Compute_id222();
    float3 interColor = frontColor.rgb + backColor.rgb;
    return BasicBlend_id400(backColor, frontColor, interColor);
}
...
void Compute_id395(inout PS_STREAMS streams)
{
    ...
    streams.shadingColor_id66 += directLightingContribution * PI_id122 + environmentLightingContribution;
    streams.shadingColorAlpha_id67 = streams.matDiffuse_id47.a;
}
void Compute_id380(inout PS_STREAMS streams)
{
    float metalness = Compute_id226(streams).r;
...
{
    float glossiness = Compute_id225(streams).r;
