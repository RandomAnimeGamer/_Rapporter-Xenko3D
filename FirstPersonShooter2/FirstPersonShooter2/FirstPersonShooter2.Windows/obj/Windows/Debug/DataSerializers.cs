...
[assembly: SiliconStudio.Core.Serialization.Serializers.AssemblySerializerFactory(Type = typeof(SiliconStudio.DataSerializers.FirstPersonShooter2SerializerFactory))]
...
namespace SiliconStudio.DataSerializers
{
    // Assembly attributes that defines supported serializer (only generics one so that other assemblies can do generic instantiations by themselves)
   	public static class FirstPersonShooter2SerializerFactory
	{
	    [SiliconStudio.Core.ModuleInitializer]
        internal static void Initialize()
        {
			var assemblySerializers = new AssemblySerializers(typeof(FirstPersonShooter2SerializerFactory).GetTypeInfo().Assembly);
			...
			assemblySerializers.Modules.Add(typeof(SiliconStudio.DataSerializers.FirstPersonShooter2_GameSerializerFactory).GetTypeInfo().Module);
            ...
            AssemblyRegistry.Register(typeof(FirstPersonShooter2SerializerFactory).GetTypeInfo().Assembly, AssemblyCommonCategories.Engine);
        }
	}
}
